...
# * constants *
# physics
sigma_SB: float = 5.670374e-5  # erg s^-1 cm^-2 K^-4
...
c: float = 29979245800  # cm/s
a: float = 4.0 * sigma_SB / c  # radiation density constant in erg cm^-3 K^-4
...
log_age_solar_system: float = np.log10(
    age_solar_system
)  # log of the age of the solar system in yr
...
NMoI_Jupiter: dict = {"Helled_2011": 0.265, "Ni_2018": 0.276}

P_Jup: float = 9.925 * 3600  # measured rotation period of Jupiter today in s
Omega_Jup: float = (
    2.0 * np.pi / P_Jup
)  # measured rotation rate of Jupiter today in s^-1
...
T_one_bar_Jup_0N: float = (
    167.3  # 1 bar temperature of Jupiter at the equator (0°N) according to Gupta et al. 2022
)
T_one_bar_Jup_12S: float = (
    170.3  # 1 bar temperature of Jupiter at 12°S according to Gupta et al. 2022
)
...
def from_flux_to_equilibrium_temperature(
...
def kappa_v_Guillot_2010(
    T_eq: float | np.ndarray | np.floating[Any],
) -> float | np.ndarray | np.floating[Any]:
...
def radiative_conductivity(
    T: float | np.ndarray | np.floating[Any],
    r: float | np.ndarray | np.floating[Any],
    kappa: float | np.ndarray | np.floating[Any],
) -> float | np.ndarray | np.floating[Any]:
    """Return the radiative conductivity for a given temperature, radius, and opacity."""
    return 64.0 * np.pi**2 * a * c * T**3 * r**4 / (3.0 * kappa)


def thermal_adjustment_timescale(
    cV: float | np.ndarray | np.floating[Any],
    conductivity: float | np.ndarray | np.floating[Any],
    M_p: float | np.ndarray | np.floating[Any],
) -> float | np.ndarray | np.floating[Any]:
    """Return the thermal adjustment timescale for a given specific heat at constant volume, conductivity, and mass."""
    return cV * M_p**2 / conductivity


def _compute_mean(f: np.ndarray, dx: np.ndarray) -> np.floating[Any]:
    """Computes the mean of f(x) for a given dx."""
    return np.dot(dx, f) / np.sum(dx)


def _integrate(
    f: np.ndarray, dx: np.ndarray, unit: str | float | None = None
) -> np.floating[Any]:

...
def single_data_mask(
    data: np.ndarray, mask_function: Callable | None = None
) -> np.ndarray:
...
def multiple_data_mask(input_data, mask_filters=None):
    ...
    # If mask_filters is a single function, apply it to all input_data
    if callable(mask_filters):
        return mask & mask_filters(*input_data)

    # Otherwise: list of filters (existing logic)
...
def sort_list_by_variable(input_list: list, variable: str | None) -> list:
    ...
    def extract_value(item):
        ...
        try:
            ...
            raise ValueError(
                f"The value of the variable '{variable}' is not a numerical value."
            )
...
def extract_lambda_expression(lambda_func):
    ...
    if "," in source[start:]:
        ...
        if source[end - 1] == ")" and source[start:end].count("(") != source[
            start:end
        ].count(")"):
            end -= 1
...
def extract_function_expression(func):
    ...
    expression[1] = (
        expression[1].split("#")[0] if "#" in expression[1] else expression[1]
    )

...
import unittest
from mesa_helper import CompositionGradient, MathProfile
...
class TestCompGrad(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.comp_grad = CompositionGradient(
            M_p=0.75, verbose=self.verbose, iso_net="planets"
        )
    ...
    def test_CompFileCreation(self):
        """Tests whether the composition file is created correctly."""

        self.comp_grad.abu_profile = lambda m, **kwargs: MathProfile.lin(
            m, m_1=0.1, m_2=0.5, f_1=1.0, f_2=0.0
        )
        self.comp_grad.create_relax_inital_composition_file(
            relax_composition_filename=self.comp_grad_file, n_bins=20
        )
